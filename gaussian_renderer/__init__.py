...
def render_active(viewpoint_camera, pc : GaussianModel_w_var, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    ...
    rasterizer = ActiveGaussianRasterizer(raster_settings=raster_settings)

    means3D = pc.get_xyz
    means2D = screenspace_points
    opacity = pc.get_opacity
    variance = pc.get_variance
    ...
    # Rasterize visible Gaussians to image, obtain their radii (on screen).
    rendered_image, rendered_var, radii = rasterizer(
        means3D = means3D,
        means2D = means2D,
        shs = shs,
        variance = variance,
    ...
    return {"render": rendered_image,
            "viewspace_points": screenspace_points,
            "variance": rendered_var,
            "visibility_filter" : radii > 0,
            "radii": radii}

def render_variance(viewpoint_camera, pc : GaussianModel_w_var, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    """
    Render the scene.
    ...
    # Rasterize visible Gaussians to image, obtain their radii (on screen).
    _, post_var, pri_var = rasterize_variance(
        means3D = means3D,
        means2D = means2D,
        variance = variance,
        shs = shs,
    ...
    # Those Gaussians that were frustum culled or had a radius of 0 were not visible.
    # They will be excluded from value updates used in the splitting criteria.
    return {"post_var": post_var, "pri_var": pri_var}
