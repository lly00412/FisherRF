...
from active.schema import override_test_idxs_dict
...
from utils.uncert_utils import plot_roc, write_auc, compute_roc, compute_ause
...
@torch.no_grad()
def render_uncertainty(view, train_cameras, gaussians, pipeline, background, hessian_color_C,args):
    ###########################
    ...
    # rgb uncertainty
    train2rd_pred_sum = train2rd_pred_imgs.sum(0).mean(0, keepdim=True)
    rendering_ = pred_img.mean(0, keepdim=True)
    train2rd_pred = torch.zeros_like(rendering_)
    train2rd_pred[numels > 0] = train2rd_pred_sum[numels > 0] / numels[numels > 0]
    rgb_l2 = (rendering_ - train2rd_pred) ** 2
    ...
    rests[f'warprf_depth'] = norm_depth_sigmas
    rests[f'warprf_rgb'] = norm_rgb_sigmas
    rests[f'warprf_depth+rgb'] = norm_rgb_sigmas + norm_depth_sigmas
...
def render_set(model_path, name, iteration, train_views, test_views, gaussians, pipeline, background, perturb_scale=1., camera_extent=None, args=None):
    ...
    # get training cameras for scene
    ...
    print()
    print()
    print(f"------train camera names: {[cam.image_name for cam in train_cameras]}")
    print()

    # # get testing cameras for scene (NOT USED, only use the test_view set...)
    # test_cameras = [cam for cam in train_views if cam.image_name not in train_cameras_names]
    # test_cameras.extend(test_views)
    ...
    with torch.no_grad():
        for idx, view in enumerate(tqdm(test_views, desc="Rendering on test set")):
            ...
            pred_img, uncertanity_map_C, pixel_gaussian_counter, depth, rests = render_uncertainty(view, train_cameras, gaussians, pipeline, background, cur_hessian_color_C, args)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--training_views", default=None, type=str, help="Path to yaml file defining the initial training views for each scene.")
    ...
    parser.add_argument("--test_idxs", nargs="+", default=None, type=int, help="index of test images to evaluate")
