...
def render_uncertainty(view, gaussians, pipeline, background, hessian_color_C,args):
    ...
    # render_path = './output/tandt/train/v20/'
...
def render_set(model_path, name, iteration, train_views, test_views, gaussians, pipeline, background, perturb_scale=1., camera_extent=None, args=None):
    ...
    if not args.depth_only:
        ...
        for idx, view in enumerate(tqdm(itertools.chain(train_views, test_views), desc="Rendering progress")):
            ...
            # torchvision.utils.save_image(pred_img.detach(), os.path.join(render_path, 'floater_remover',f"{split}_{view.image_name}.png"))
    else:
        H_per_gaussian_C += 1
    ...
    with torch.no_grad():
        for idx, view in enumerate(tqdm(test_views, desc="Rendering on test set")):
            ...
            for q in q_pts:
                ...
                mask = (vcu>u_q)
                current_psnr = psnr(gt_img[:,~mask], pred_img[:,~mask]).mean()
                print(f'q: {q}, psnr: {current_psnr}')
                filter_img = pred_img.clone()
                filter_img = torch.clamp(filter_img, 0, 1)
                filter_img[:,mask] = 1.
                torchvision.utils.save_image(filter_img.detach(), os.path.join(floter_path, 'vcu',f"{split}_{view.image_name}_{q}_psnr_{current_psnr}.png"))

            fisher = uncertanity_map_C
            for q in q_pts:
                u_q = torch.quantile(fisher.view(-1), q)
                mask = (fisher > u_q)
                current_psnr = psnr(gt_img[:, ~mask], pred_img[:, ~mask]).mean()
                print(f'q: {q}, psnr: {current_psnr}')
                filter_img = pred_img.clone()
                filter_img = torch.clamp(filter_img, 0, 1)
                filter_img[:, mask] = 1.
                torchvision.utils.save_image(filter_img.detach(), os.path.join(floter_path, 'fisherRF', f"{split}_{view.image_name}_{q}_psnr_{current_psnr}.png"))
