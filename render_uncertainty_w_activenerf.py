...
from gaussian_renderer import GaussianModel_w_var as GaussianModel
import numpy as np
from utils.camera_utils import rand_rotation_matrix
from scene.cameras import Camera
from gaussian_renderer import render_active, render_variance
...
def render_uncertainty(view, gaussians, pipeline, background, hessian_color_C,args):
    ...
    ###########################
    rests = {}
    render_pkg = render_active(view, gaussians, pipeline, background)
        ...
        for N in args.n_vcam:
            for scale in args.r_scale:
                ...
                for vir_view in Vcams:
                    vir_render_pkg = render_active(vir_view, gaussians, pipeline, background)
...
def render_set_current(model_path, name, iteration, train_views, test_views, gaussians, pipeline, background, perturb_scale=1., camera_extent=None, args=None):
    ...
    for idx, view in enumerate(tqdm(test_views, desc="Rendering on test set")):
        ...
        with torch.no_grad():
            ...
            render_pkg = render_active(view, gaussians, pipeline, background, override_color=hessian_color)
