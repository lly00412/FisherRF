...
class VNSeqMReplace(BaseSchema):
    """
    Add 1 image at a time
    """

    def __init__(self, dataset_size: int, scene, M: int = 1, num_init_views: int = 1, interval_epochs=100,
                 **kwargs):
        """
        N: int total views to select
        M: # views to select each time
        ...
        cur_dataset_size = len(self.init_views)
        it_base = cur_dataset_size * interval_epochs
        num_views_left = dataset_size - len(self.init_views)
        ...
        if num_views_left > 0:
            ...
            while num_views_left > 0:
                ...
                cur_dataset_size = M + len(self.init_views)
                it_base += cur_dataset_size * interval_epochs
                num_views_left -= M
