...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    ...
    if hasattr(args,"train_idxs"):
        ...
        train_idxs = None
    # For emsembling training
    if hasattr(args,"n_emsemble"):
        n_emsemble = args.n_emsemble
    else:
        train_idxs = None
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if args.method == "variance":
            ...
            if cur_iter % opt.opacity_reset_interval <= 200 :
                ...
                depth_l2,rgb_l2, nv_mask = render_vcam_difference_v2(render_pkg, viewpoint_cam, gaussians, pipe, background,
                                              n_vcam=args.n_vcam,r_scale=args.r_scale,method='vcam') # (n_vcam, 4, h, w)
                loss1 = loss1+ 0.001*rgb_l2.mean()
                loss1.backward()
                diff = torch.cat([depth_l2.detach(), rgb_l2.detach()], dim=1)
                _, h, w = image.shape
                ...
                # NLL + SSIM Loss
                loss2 = (image.detach() - gt_image) ** 2 / (2*sigmas) + torch.log(sigmas) / 2
                loss2 = loss2.mean()
                loss2.backward()
                loss = loss1 + loss2
                # loss = (1.0 - opt.lambda_dssim) * error.mean() + opt.lambda_dssim * (1.0 - ssim(image, gt_image))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--emsemble_seed", type=int, default=0)
